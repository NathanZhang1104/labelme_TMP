...
from labelme.widgets.thre_widget import ThreWight,cvImgtoQtImg,get_hull_fromimg
# FIXME
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.thre_dock=QtWidgets.QDockWidget(self.tr("Thre"), self)
        self.thre_widget=ThreWight()
        self.thre_dock.setWidget(self.thre_widget)
        ...
        self.canvas.rectangle_sig.connect(self.show_thre)
        self.setCentralWidget(scrollArea)

        features = QtWidgets.QDockWidget.DockWidgetFeatures()
        for dock in ["flag_dock", "label_dock", "shape_dock", "file_dock","thre_dock"]:
        ...
        self.addDockWidget(Qt.RightDockWidgetArea, self.thre_dock)
        ...
        self.labelDialog.slider.sliderReleased.connect(lambda :self.change_thre( self.labelDialog.slider.value()))
    ...
    def change_thre(self,kmeans_z):
        ...
        if (len(qp_list) == 2):
            ...
            #
            try:
                poslist = [qp_list[0].x(), qp_list[1].x(), qp_list[0].y(), qp_list[1].y()]
                xl = [int(i) for i in poslist[0:2]]
                yl = [int(i) for i in poslist[2:4]]
                xl.sort()
                yl.sort()
                cur_img = self.image_cv[yl[0]:yl[1], xl[0]:xl[1]]
                th = None
                shapes_list, th = get_hull_fromimg(cur_img, kmeans_z, (xl[0], yl[0]))

                del self.canvas.shapes[-self.tmp_hull_count-1:-1]
                self.tmp_hull_count = len(shapes_list)
                for shape in shapes_list:
                    self.canvas.shapes.insert(-1,shape)
                self.canvas.update()

                self.canvas.storeShapes()
                x, y = cur_img.shape[0:2]
                # 缩放到原来的二分之一，输出尺寸格式为（宽，高）
                # img_test1 = cv2.resize(th, (int(y / 3), int(x / 3)))
                qimg = cvImgtoQtImg(th)
                self.thre_widget.setpixmap(qimg)
            except:
                print(traceback.format_exc())
    def show_thre(self,qp_list):
        if(len(qp_list)==2):
            # return
            #
            ...
            try:
                ...
                cur_img=self.image_cv[yl[0]:yl[1],xl[0]:xl[1]]
                th=None
                shapes_list,th=get_hull_fromimg(cur_img,4,(xl[0],yl[0]))
                self.tmp_hull_count=len(shapes_list)
                for shape in shapes_list:
    ...
    def newShape(self):
        ...
        if text:
            ...
            shape.group_id = group_id
            if (not self.canvas.createMode == "rectangle"):
                self.addLabel(shape)
            ...
            if (self.canvas.createMode == "rectangle"):
                ...
                for i in range(-self.tmp_hull_count, 0):
                    self.canvas.shapes[i].label = "hull"
                    self.canvas.shapes[i].flags = {}
                    self.addLabel(self.canvas.shapes[i])
        else:
            del self.canvas.shapes[-self.tmp_hull_count - 1:-1]
